...
# 让受击反应动画不打断攻击动画
创建一个BlockHitReact的 GE 和 tag
给GE添加一个Components 里面选择授予Tag GrantTagsToTargetActor
在攻击GA触发时,给自己授予这个Effect,GA结束时移除GE(移除可以使用函数 RemoveGameplayEffectFromOwnerWithGrantedTags)
在触发受击GA中增加判断,使用HasMatchingGameplayTag 可以判断ASC上有没有此Tag,决定是否播放受击动画